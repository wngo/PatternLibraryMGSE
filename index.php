
<?php
// Build out URI to reload from form dropdown
// Need full url for this to work in Opera Mini
$pageURL = (@$_SERVER["HTTPS"] == "on") ? "https://" : "http://";
if (isset($_POST['sg_uri']) && isset($_POST['sg_section_switcher'])) {
$pageURL .= $_POST[sg_uri].$_POST[sg_section_switcher];
$pageURL = htmlspecialchars( filter_var( $pageURL, FILTER_SANITIZE_URL ) );
header("Location: $pageURL");
}
// Display title of each markup samples as a select option
function listMarkupAsOptions ($type) {
$files = array();
$handle=opendir('markup/'.$type);
while (false !== ($file = readdir($handle))):
if(stristr($file,'.html')):
$files[] = $file;
endif;
endwhile;
sort($files);
foreach ($files as $file):
$filename = preg_replace("/\.html$/i", "", $file);
$title = preg_replace("/\-/i", " ", $filename);
$title = ucwords($title);
echo '<option value="#sg-'.$filename.'">'.$title.'</option>';
endforeach;
}
// Display markup view & source
function showMarkup($type) {
$files = array();
$handle=opendir('markup/'.$type);
while (false !== ($file = readdir($handle))):
if(stristr($file,'.html')):
$files[] = $file;
endif;
endwhile;
sort($files);
foreach ($files as $file):
$filename = preg_replace("/\.html$/i", "", $file);
$title = preg_replace("/\-/i", " ", $filename);
$documentation = 'doc/'.$type.'/'.$file;
echo '<div class="sg-markup sg-section">';
echo '<div class="sg-display">';
echo '<h2 class="sg-h2"><a id="sg-'.$filename.'" class="sg-anchor">'.$title.'</a></h2>';
if (file_exists($documentation)) {
echo '<div class="sg-doc">';
echo '<h3 class="sg-h3">Comments</h3>';
include($documentation);
echo '</div>';
}
echo '<h3 class="sg-h3">Example</h3>';
include('markup/'.$type.'/'.$file);
echo '</div>';
echo '<div class="sg-markup-controls"><a class="sg-btn sg-btn--source" href="#">View Source</a> <a class="sg-btn--top" href="#top">Back to Top</a> </div>';
echo '<div class="sg-source sg-animated">';
echo '<a class="sg-btn sg-btn--select" href="#">Copy Source</a>';
echo '<pre class="prettyprint linenums"><code>';
echo htmlspecialchars(file_get_contents('markup/'.$type.'/'.$file));
echo '</code></pre>';
echo '</div>';
echo '</div>';
endforeach;
}
?>
<!DOCTYPE html>
<head>
<meta charset="utf-8">
  <title>Style Guide Boilerplate</title>
  <meta name="viewport" content="width=device-width">
  <!-- Style Guide Boilerplate Styles -->
  <link rel="stylesheet" href="css/sg-style.css">
  
  <!-- Replace below stylesheet with your own stylesheet -->
  <link media="all" href="http://education.unimelb.edu.au/_mappdesign/mgsestaging_design2.css" type="text/css" rel="stylesheet">
  <link href="//maxcdn.bootstrapcdn.com/font-awesome/4.2.0/css/font-awesome.min.css" rel="stylesheet">

  <script src="https://code.jquery.com/jquery-2.1.1.min.js"></script>
  <script src="http://fast.fonts.net/jsapi/7e9801db-a894-431d-8544-b85ad9cf5f1e.js" type="text/javascript"></script>
</head>
<body class="no-bg"> 
<div id="top-header" class="sg-header sg-container">
  <h1 class="sg-logo">MGSE Components Guide</h1>
  <form id="js-sg-nav" action=""  method="post" class="sg-nav">
    <select id="js-sg-section-switcher" class="sg-section-switcher" name="sg_section_switcher">
        <option value="">Jump To Section:</option>
        <optgroup label="Intro">
          <option value="#sg-colors">Colors</option>
        </optgroup>
        <optgroup label="Base Styles">
          <?php listMarkupAsOptions('base'); ?>
        </optgroup>
        <optgroup label="Pattern Styles">
          <?php listMarkupAsOptions('patterns'); ?>
        </optgroup>
        <optgroup label="Grid Styles">
          <?php listMarkupAsOptions('grids'); ?>
        </optgroup>
    </select>
    <input type="hidden" name="sg_uri" value="<?php echo $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"]; ?>">
    <button type="submit" class="sg-submit-btn">Go</button>
  </form><!--/.sg-nav-->
</div><!--/.sg-header-->

<div class="sg-body sg-container">
  <div class="sg-info">               
    
    <div class="sg-colors sg-section" id-"top">
      <h2 class="sg-h2"><a id="sg-colors" class="sg-anchor">Colours currently used on the web</a></h2>
        <div class="sg-color sg-color--a"><span class="sg-color-swatch"><span class>#ffffff</span></span></div>
        <div class="sg-color sg-color--f"><span class="sg-color-swatch" style="background:#e3f3ff;"><span class>#e3f3ff</span></span></div>
        <div class="sg-color sg-color--hover"><span class="sg-color-swatch" style="background:#0072c9;"><span class>#0072c9</span></span></div>
        <div class="sg-color sg-color--c"><span class="sg-color-swatch"><span class>#005596</span></span></div>
        <div class="sg-color sg-color--d"><span class="sg-color-swatch"><span class>#00457c</span></span></div>
        <div class="sg-color sg-color--e"><span class="sg-color-swatch" style="background:#4a4a4a;"><span class>#4a4a4a</span></span></div>
<!-- <div class="sg-color sg-color--f"><span class="sg-color-swatch"><span class>#badef5</span></span></div> -->
        <div class="sg-markup-controls"><a class="sg-btn--top" href="#top">Back to Top</a></div>

<h2 class="sg-h2"><a id="sg-colors" class="sg-anchor">Official MGSE colours</a></h2>
        <div class="sg-color sg-color--blue"><span class="sg-color-swatch"><span class>#00447c</span></span></div>
        <div class="sg-color sg-color--grey"><span class="sg-color-swatch"><span class>#c4c4c5</span></span></div> 
        <div class="sg-color sg-color--black"><span class="sg-color-swatch"><span class>#20201e</span></span></div>
        
        <!-- <div class="sg-color sg-color--f"><span class="sg-color-swatch"><span class>#badef5</span></span></div> -->
        <div class="sg-markup-controls"><a class="sg-btn--top" href="#top">Back to Top</a></div>



        <h2 class="sg-h2"><a id="sg-colors" class="sg-anchor">Shard Colours</a></h2>
        <div class="sg-color sg-shard-color--a"><span class="sg-color-swatch"><span class>#B7D1ED</span></span></div>
        <div class="sg-color sg-shard-color--b"><span class="sg-color-swatch"><span class>#009DDC</span></span></div>
        <div class="sg-color sg-shard-color--c"><span class="sg-color-swatch"><span class>#026CB6</span></span></div>
        <div class="sg-color sg-shard-color--d"><span class="sg-color-swatch"><span class>#00447C</span></span></div>
        <div class="sg-markup-controls"><a class="sg-btn--top" href="#top">Back to Top</a></div>

    </div>

    <!--/.sg-colors-->
    



  </div><!--/.sg-info-->    

  <div class="sg-base-styles">    
    <h1 class="sg-h1">Base Styles</h1>
    <?php showMarkup('base'); ?>
  </div><!--/.sg-base-styles-->

  <div class="sg-pattern-styles">
    <h1 class="sg-h1">Pattern Styles<small> - Design and mark-up patterns.</small></h1>
    <?php showMarkup('patterns'); ?>
    </div><!--/.sg-pattern-styles-->

  <div class="sg-grid-styles">
    <h1 class="sg-h1">Grid Styles<small> - Grid patterns.</small></h1>
    <?php showMarkup('grids'); ?>
    </div><!--/.sg-pattern-styles-->


  </div><!--/.sg-body-->


  <script src="js/sg-plugins.js"></script>
  <script src="js/sg-scripts.js"></script>
<script src="http://education.unimelb.edu.au/__data/assets/js_file/0003/1159257/banner-roller.js"></script>
<!--these scripts should be kept below the slider stuff or it will break-->
<!--collapsible-->
<script src="http://education.unimelb.edu.au/_design/js/jquery.collapsible.min.js"></script>
<!--js expander-->
<script src="http://education.unimelb.edu.au/__data/assets/js_file/0011/590906/jquery.expander.min.js"></script>
<!--fitvid-->
<script src="http://education.unimelb.edu.au/__data/assets/js_file/0009/1070388/fitVid.js"></script>

<!--lazy sizes-->
<script src="http://education.unimelb.edu.au/__data/assets/js_file/0010/1263277/lazysizes.min.js"></script>

<script src="http://education.unimelb.edu.au/__data/assets/js_file/0018/1063035/respsonsiveTabs.js"></script>
<!-- mixitup -->
<script src="http://cdn.jsdelivr.net/jquery.mixitup/latest/jquery.mixitup.min.js"></script>
<script>
jQuery(document).ready(function() {

$("#main-content").fitVids();

$('.abstract').expander({
        slicePoint:       100,  // default is 100
        expandPrefix:     '...', // default is '... '
        detailClass: 'more-details',
        summaryClass: 'summary',
        expandText: 'read more',
        expandEffect: 'show',
        expandSpeed: 0,
        collapseEffect: 'hide',
        collapseSpeed: 0
        });

 var type = window.location.hash.substr(1);

 $('.collapsible').collapsible({
 defaultOpen: type,
 speed: 'fast'
 });

});
</script>

<script>
jQuery(document).ready(function() {

$("#main-content").fitVids();

$('.abstract').expander({
        slicePoint:       100,  // default is 100
        expandPrefix:     '...', // default is '... '
        detailClass: 'more-details',
        summaryClass: 'summary',
        expandText: 'read more',
        expandEffect: 'show',
        expandSpeed: 0,
        collapseEffect: 'hide',
        collapseSpeed: 0
        });

 var type = window.location.hash.substr(1);

 $('.collapsible').collapsible({
 defaultOpen: type,
 speed: 'fast'
 });

 $("ul.tabs").tabs("div.panes > div"); 
});

</script>
<script type="text/javascript">
  $(document).ready(function() {
 
// Instantiate MixItUp:

  $('#container').mixItUp({
      animation: {
          effects: 'fade',
          duration: 700
      }

  });

  $('#container').mixItUp({
  layout: {
    containerClassFail: 'none-found'
  }
});

$('#container').mixItUp('insert', 5, $('<div class="d-all m-all t-all" aria-hidden="true"><p class="hide">Sorry no events to show for this category.</p></div>'), {filter: 'all'})

});

</script>

<style>
.show-grid {
    background-color: #badef5;
    border: 1px solid #badef5;
    padding-bottom: 10px;
    padding-top: 10px;
}
</div>
</body>
</html>
 